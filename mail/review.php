...
class MailSender
{
     ...
     * Тема письма
     */
    const MAIL_SUBJECT = "Заявка с сайта врачебный офис (отзыв)";
    ...
    private function getMessage()
    {
        ...
        $message .= '<div> Фамилия: ' . $this->getParam('surname') . '</div>';
        $message .= '<div> Отчество: ' . $this->getParam('surname2') . '</div>';
        ...
        $message .= '<div> Категория: ' . $this->getParam('category') . '</div>';
        $message .= '<div> Рейтинг: ' . $this->getParam('rating') . '</div>';
        $message .= '<div>-------------------------------------------------------------</div> </br></br> ';
        return $message;
    }
